  ...
  const { isAuthenticated, authenticate, user, logout, Moralis, enableWeb3 } = useMoralis()
    ...
    useEffect(() => {
      if (isAuthenticated) {
       ...
       /*  this is where the error occured const currentBalance = 
       await Moralis.Web3API.account.getNativeBalance({
          chain: 'rinkeby',
          address: currentAccount,
        }) */

       /* ALSO CANNOT ADD THIS LINE DUE TO ERROR ABVE: const balanceToEth = Moralis.Units.FromWei(currentBalance.balance)
